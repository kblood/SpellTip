...
-- edit
  local costPerSecond, powerType, spellCost = 0
  local name, rank, icon, castTime, minRange, maxRange, spellId = GetSpellInfo(id)
  local costTable = GetSpellPowerCost(id)
  
  if name and kind == kinds.spell then 
      for _, costInfo in pairs(costTable) do
        powerType = costInfo.name;
        spellCost = costInfo.cost;
        break;
      end

    if spellCost then
      local spellData = TheoryCraft_GetSpellDataByDescription(GetSpellDescription(id))
      -- local spellData = TheoryCraft_GetSpellDataByName(name, rank)
      if spellData then
        for k,v in pairs(spellData) do 
          if k == "hpm" and v > 0 then
            left = NORMAL_FONT_COLOR_CODE .. "Healing per " .. powerType .. " :" .. FONT_COLOR_CODE_CLOSE
            right = HIGHLIGHT_FONT_COLOR_CODE .. round(v, 2) .. FONT_COLOR_CODE_CLOSE
            tooltip:AddDoubleLine(left, right)
          end
          if k == "withdotdpm" and v > 0 then
            left = NORMAL_FONT_COLOR_CODE .. "Damage with dot per " .. powerType .. " :" .. FONT_COLOR_CODE_CLOSE
            right = HIGHLIGHT_FONT_COLOR_CODE .. round(v, 2) .. FONT_COLOR_CODE_CLOSE
            tooltip:AddDoubleLine(left, right)
          end
          if k == "dpm" and v > 0 then
            left = NORMAL_FONT_COLOR_CODE .. "Damage per " .. powerType .. " :" .. FONT_COLOR_CODE_CLOSE
            right = HIGHLIGHT_FONT_COLOR_CODE .. round(v, 2) .. FONT_COLOR_CODE_CLOSE
            tooltip:AddDoubleLine(left, right)
          end
          if k == "dps" and v > 0 then
            left = NORMAL_FONT_COLOR_CODE .. "Damage per second:" .. FONT_COLOR_CODE_CLOSE
            right = HIGHLIGHT_FONT_COLOR_CODE .. round(v, 2) .. FONT_COLOR_CODE_CLOSE
            tooltip:AddDoubleLine(left, right)
          end
          if k == "hps" and v > 0 then
            left = NORMAL_FONT_COLOR_CODE .. "Healing per second:" .. FONT_COLOR_CODE_CLOSE
            right = HIGHLIGHT_FONT_COLOR_CODE .. round(v, 2) .. FONT_COLOR_CODE_CLOSE
            tooltip:AddDoubleLine(left, right)
          end
          --  print(k,v)
      ...
      -- costPerSecond = spellCost / castTime
      -- left = NORMAL_FONT_COLOR_CODE .. powerType .. " cost per second :" .. FONT_COLOR_CODE_CLOSE
      -- right = HIGHLIGHT_FONT_COLOR_CODE .. round(costPerSecond, 4)*1000 .. FONT_COLOR_CODE_CLOSE
    
      -- tooltip:AddDoubleLine(left, right)
    end
    --print(GetSpellDescription(id))
  end
  -- edit end
  --print(GetSpellDescription(id))
